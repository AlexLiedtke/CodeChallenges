...
public class BinaryTreeProblems {
    ...
    public static void Main() {
		...
		Console.WriteLine(DepthSum(arr[0]));  //Should be 16
		Console.WriteLine(DepthSum(arr[3]));  //Should be 2
		Console.WriteLine(DepthSum(arr[1]));  //Should be 6
		Console.WriteLine(DepthSum(arr[5]));  //Should be 0
		...
		Console.WriteLine(AllDepthSums(arr[0]));  //Should be 26
		Console.WriteLine(AllDepthSums(arr[1]));  //Should be 8
		
		//Problem #3:
		//  Continuing with the theme, write a function where you pass it a node and it returns the sum of the distances from all other nodes in the tree.
		//  In the setup tree this would mean DistanceSum(Node 0) = 16 still, but if you called DistanceSum(Node 1) the answer would be 15, since Node 0 is
		//  a distance of 1 from Node 1, Node 2 is distance 2, and Nodes 5 and 6 are distance 3
		Console.WriteLine(DistanceSum(arr[0]));  //Should be 16
		Console.WriteLine(DistanceSum(arr[1]));  //Should be 15
		Console.WriteLine(DistanceSum(arr[4]));  //Should be 22
		Console.WriteLine(DistanceSum(arr[7]));  //Should be 25
    }
	...
	//ANSWER #3:
	public static int DistanceSum(Node currentNode){
		//TODO: Write function
		return 0;
	}
